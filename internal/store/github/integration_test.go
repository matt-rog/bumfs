...
func TestIntegration(t *testing.T) {
	...
	bc, ok := cfg.Backends["github"]
	if !ok || bc.Type != "github" {
		t.Skip("skipping: no github backend configured")
	}

	dbPath := filepath.Join(t.TempDir(), "index.json")
	b, err := New(bc.Token, bc.Owner, bc.Repo, dbPath)
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	...
	payload := []byte("bumfs github integration test payload")
