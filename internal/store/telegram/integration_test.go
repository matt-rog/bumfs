...
func TestIntegration(t *testing.T) {
	...
	bc, ok := cfg.Backends["telegram"]
	if !ok || bc.Type != "telegram" {
		t.Skip("skipping: no telegram backend configured")
	}

	dbPath := filepath.Join(t.TempDir(), "index.json")
	b, err := New(bc.BotToken, bc.ChatID, dbPath)
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	...
	payload := []byte("bumfs telegram integration test payload")
